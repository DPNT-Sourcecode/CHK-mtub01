...
const reductionSchemes = {
    ...
    'grouppack': (offerString, price) => {
        if (offerString) {
            ...
            let amount = null
            let group = null
            console.log('Group pack',stripData,offerString)
            if (stripData && stripData.length == 4) {
                try {
                    quantity = parseInt(stripData[1])
                    group = stripData[2].split(',')
                    offerPrice = parseInt(stripData[3].split(','))
                    groupData = { group: group, quantity: quantity, totalPrice: offerPrice }
                    return groupData
                } catch (exception) {
