...
const reductionSchemes = {
    ...
    'freepack': (offerString) => {
        if (offerString)
        {
            const copyString=wordToNumbers(offerString)
            const stripQuantityFromOffer = offerString.match(/(\d+)\w+ get/)
            const stripAmountFromOffer = offerString.match(/(\d+) (\w+) free$/)
            let quantity = null
            let amount = null
            let freeItem=null
            ...
            if (quantity && freeItem && freeQuantity) {
                const freeObj={free:freeItem,quantity:freeQuantity}
